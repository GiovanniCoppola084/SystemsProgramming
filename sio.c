...
**      queue operations:  QCREATE, QLENGTH, QDEQUE
**      sio read queue:  QNAME
**
** Our SIO scheme is very simple:
**
**  Input:  We maintain a buffer of incoming characters that haven't
**      yet been read by processes.  Optionally, processes that
**      attempt to read when there are no characters available can
**      be blocked on a read queue; this is controlled by the QNAME
**      macro defined in "compat.h".
**
**      When a character comes in, if there is no process waiting for it,
**      it goes in the buffer; otherwise, the first waiting process is
**      awakened and it gets the character.
**
**      When a process invokes readch(), if there is a character in
**      the input buffer, the process gets it; otherwise, it is
**      either blocked until input appears, or it receives a "no
**      characters available" return value.
**
...
**      characters in the buffer, _sio_readc() returns a -1
...
// compatibility definitions; also includes all other standard
// headers for this baseline system
#include "compat.h"
    ...
    for(;;) {
        ...
        case UA4_EIR_RX_INT_PENDING:
...
#if defined(QNAME)
            // This code is compiled in only if the OS queues
            // processes that block when there are no characters
            // available to read from the SIO
            ...
            if( QLENGTH(QNAME) > 0 ) {
                ...
                // pull the process off the queue
                Payload p = { 0, 0 };
                QDEQUE( QNAME, p );
                ...
                // make the PCB pointer usable
                pcb = (Pcb *) p.data;

                // return char via arg #2 and count in EAX
                // WARNING: this crosses the boundary between kernel
                // and user address spaces.  If the OS is modified
                // to do address space separation, this will need
                // to be updated!
...
#if TRACING_SIO_STAT
    __cio_printf( "** SIO enable %02x, curr %02x\n", which, _ier );
#endif
...
#if TRACING_SIO_STAT
    __cio_printf( "** SIO enable, now %02x\n", _ier );
#endif
...
#if TRACING_SIO_STAT
    __cio_printf( "** SIO disable %02x, curr %02x\n", which, _ier );
#endif
    // remember the current status
...
#if TRACING_SIO_STAT
    __cio_printf( "** SIO disable, now %02x\n", _ier );
#endif
...
int _sio_reads( char *buf, int length ) {
    ...
    if( _incount < 1 ) {
#if TRACING_SIO_RD
    __cio_printf( "** SIO reads, buf 0x%08x len %d, EMPTY\n",
            (uint32_t) buf, length );
#endif
        return( 0 );
    }
...
#if TRACING_SIO_RD
    __cio_printf( "** SIO reads, buf 0x%08x len %d, ret %d\n",
            (uint32_t) buf, length, copied );
#endif
...
#if TRACING_SIO_WR
    __cio_printf( "** SIO write, buf 0x%08x len %d\n",
