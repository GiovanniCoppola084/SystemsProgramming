...
USERMAIN( idle ) {
    ...
    // create_inode(fs, fs->current_inode, index_into_inode, false, "Direc 2");

    // sprint(str, "Address of Direc 2: %08x\n", (Inode_s *)fs->current_inode->direct[index_into_inode]);
    // cwrites(str);
    // DELAY(LONG * 20);
    ...
    // index_into_inode = 0;
    // inode = (Inode_s *)(fs->current_inode->direct[index_into_inode]);
    // delete_pointer_in_inode(fs, fs->current_inode, index_into_inode, false, inode->name);
    // print_directory(fs, fs->current_inode);
    // DELAY(LONG * 20);
